...
protected:
	...
	QImage screen;
public:
	Renderer(const int& width_scrn_, const int& height_scrn_, const Camera& c_, const Vector3& light_, Terrain * t_) : width_scrn(width_scrn_), height_scrn(height_scrn_), c(c_), light(light_), t(t_), screen(QImage(width_scrn_, height_scrn_, QImage::Format::Format_RGB32)){};
		...
		for (int i = 0; i < width_scrn; i++)
		{
			for (int j = 0; j < height_scrn; j++)
			{
				...
				if (f > Constante::noIntersect && intersect.z >= t->getLow()) //Si intersection
				{
					...
					//intersect += lightvec.getDirection()  * (eps);
